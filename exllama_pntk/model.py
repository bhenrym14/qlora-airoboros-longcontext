...
class ExLlamaConfig:
    ...
    def __init__(self, model_config_path):
        ...
        self.bos_token_id = read_config[
            "bos_token_id"]  # Note that the HF LlamaTokenizer doesn't seem to recognize these automatically
        ...
        try:
            self.original_max_position_embeddings = read_config["original_max_position_embeddings"]
        except KeyError:
            self.original_max_position_embeddings = None
        self.max_position_embeddings = read_config["max_position_embeddings"]
...
class ExLlamaMLP:
    ...
    def __init__(self, config, tensors, key):
        ...
        self.gate_proj = Ex4bitLinear(config, self.config.hidden_size, self.config.intermediate_size, False, tensors,
                                      key + ".gate_proj")
        self.up_proj = Ex4bitLinear(config, self.config.hidden_size, self.config.intermediate_size, False, tensors,
                                    key + ".up_proj")
        self.down_proj = Ex4bitLinear(config, self.config.intermediate_size, self.config.hidden_size, False, tensors,
                                      key + ".down_proj")
...
class ExLlamaAttention:
    ...
    def __init__(self, config, tensors, key, sin, cos, index):
        ...
        self.q_proj = Ex4bitLinear(config, self.config.hidden_size,
                                   ...
                                   key + ".q_proj")
        self.k_proj = Ex4bitLinear(config, self.config.hidden_size,
                                   self.config.num_key_value_heads * self.config.head_dim, False, tensors,
                                   key + ".k_proj")
        self.v_proj = Ex4bitLinear(config, self.config.hidden_size,
                                   self.config.num_key_value_heads * self.config.head_dim, False, tensors,
                                   key + ".v_proj")
        self.o_proj = Ex4bitLinear(config, self.config.num_attention_heads * self.config.head_dim,
                                   self.config.hidden_size, False, tensors, key + ".o_proj")
    ...
    def fused(self, hidden_states, cache, buffer, input_layernorm, lora):
        ...
        query_states = torch.empty((bsz, q_len, self.config.num_attention_heads * self.config.head_dim),
                                   dtype=torch.float16, device=hidden_states.device)
        key_states = torch.empty((bsz, q_len, self.config.num_key_value_heads * self.config.head_dim),
                                 dtype=torch.float16, device=hidden_states.device)
        value_states = torch.empty((bsz, q_len, self.config.num_key_value_heads * self.config.head_dim),
                                   dtype=torch.float16, device=hidden_states.device)
    ...
    def forward(self, hidden_states, cache, buffer, lora):
        ...
        cuda_ext.exllama_ext.rope_(query_states, self.sin, self.cos, past_len, self.config.num_attention_heads,
                                   self.config.head_dim)
        cuda_ext.exllama_ext.rope_(key_states, self.sin, self.cos, past_len, self.config.num_key_value_heads,
                                   self.config.head_dim)

        query_states = query_states.view(bsz, q_len, self.config.num_attention_heads, self.config.head_dim).transpose(1,
                                                                                                                      2)
        key_states = key_states.view(bsz, q_len, self.config.num_key_value_heads, self.config.head_dim).transpose(1, 2)
        value_states = self.v_proj.forward(hidden_states, lora).view(bsz, q_len, self.config.num_key_value_heads,
        ...
        # -- Scaled dot-product attention from PyTorch 2, should be comparable to xformers (?)
            ...
            if past_len > 0 or (bsz > 1 and buffer.attn_mask is not None):
                attn_output = F.scaled_dot_product_attention(query_states, key_states, value_states,
                                                             attn_mask=buffer.attn_mask, is_causal=False)
            else:
                attn_output = F.scaled_dot_product_attention(query_states, key_states, value_states, attn_mask=None,
                                                             is_causal=True)
...
class ExLlamaCache:
    ...
    def __init__(self, model, batch_size=1, max_seq_len=-1, copy_from=None):
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            if copy_from is None:
                ...
                p_key_states = torch.zeros(self.batch_size, self.config.num_key_value_heads, self.max_seq_len,
                                           self.config.head_dim, dtype=torch.float16,
                                           device=self.model.config.device_map.layers[i])
                p_value_states = torch.zeros(self.batch_size, self.config.num_key_value_heads, self.max_seq_len,
                                             self.config.head_dim, dtype=torch.float16,
                                             device=self.model.config.device_map.layers[i])
    ...
    def copy_states(self, target, from_column, from_columns, to_column, to_columns, from_row, from_rows, to_row,
                    to_rows):
...
class ExLlamaBuffer:
    ...
    def to(self, device):
        ...
        new.attn_mask = None if self.attn_mask is None else _move_tensor(self.attn_mask, device, "attn_mask",
                                                                         self.config)
        return new
...
class ExLlama:
    ...
    def __init__(self, config):
            ...
            if self.config.auto_map is not None:
                ...
                for layer in range(self.config.num_hidden_layers + 2):
                    ...
                    while device_usage + this_layer_size > max_usage:
                        ...
                        layer_index_device = 0
                        max_usage = self.config.auto_map[device_index] * (1024 ** 3)
                        if device_index >= len(self.config.auto_map): raise ValueError(
        ...
        for device in devs:
            ...
            if self.config.original_max_position_embeddings is not None:
                beta_0 = 1.25
                beta_1 = 0.75
                gamma_0 = 16
                gamma_1 = 2
                ...
                print(f"Using NTK by parts scale factor of {scale}")
                ...
                inv_freq_ntk = 1.0 / (
                        find_newbase_ntk(dim, base, scale) ** (torch.arange(0, dim, 2).float().to(device) / dim))
                ...
                inv_freq = 1.0 / (self.config.rotary_embedding_base ** (
        ...
        for dev in self.config.device_map.get_layers_devs():
            ...
            temp_mlp = torch.zeros((config.fused_mlp_thd * 2, config.intermediate_size), dtype=torch.float16,
                                   device=dev)
    ...
    def forward(self,
        ...
        assert input_mask is None or (
                    input_mask.shape[-1] >= input_ids.shape[-1] and input_mask.shape[-2] == input_ids.shape[-2])
