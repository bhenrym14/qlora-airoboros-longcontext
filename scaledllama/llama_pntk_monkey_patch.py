"""Adapted from https://github.com/jquesnelle/scaled-rope"""
...
class LlamaDynamicPartNTKScaledRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, original_max_position_embeddings=2048, base=10000, ntk_factor=1, extrapolation_factor=1, finetuned=True, device=None):
        super().__init__()
        self.dim = dim
        self.base = base
        ...
        if finetuned:
            print(f"finetuning = True. Calculating implied inv freq for max pos {self.max_position_embeddings} for scaling factor of {self.max_position_embeddings / original_max_position_embeddings}")
            self.ntk(self.max_position_embeddings / original_max_position_embeddings, device)
        else:
            print(f"PNTK Finetune Mode = False. Dynamic scaling will occur.")
            inv_freq = 1.0 / \
...
# monkey_patch. After transformers has been imported insert the following two lines (adjusting import for where you placed this file):

# from llama_pntk_monkey_patch import LlamaDynamicPartNTKScaledRotaryEmbedding
# transformers.models.llama.modeling_llama.LlamaRotaryEmbedding = LlamaDynamicPartNTKScaledRotaryEmbedding